    ...
    <div class="mainEdit">
      ...
      >
        <el-form-item label="客户名称：" prop="name">
          <el-input v-model="customerForm.name" placeholder="填写客户名称" disabled />
        </el-form-item>
...
export default {
  ...
  mounted () {
    ...
    this.customerForm.sex = allInfo.sex
    this.customerForm.phoneNum = allInfo.phone
  ...
  methods: {
    submit (formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          Server.changeCustomer(params).then((res) => {
            if (res.code === 0) {
