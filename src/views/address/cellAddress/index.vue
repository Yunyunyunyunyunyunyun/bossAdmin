  ...
  <div class="cellAddress">
    ...
    <div class="firstContent">
      <el-row>
        ...
        <el-col :span="6">
          <div class="topDiv">
            <span class="topTitle">小区：</span>
            <el-input
              v-model="plot" placeholder="请输入小区名称" clearable
              size="medium"
            />
          </div>
        </el-col>
    ...
    <div>
      ...
      >
        ...
        <el-table-column prop="housingEstate" label="小区" align="center" />
        <el-table-column prop="peoples" label="总户数" align="center" />
        <el-table-column prop="carNumber" label="开卡数" align="center" />
    ...
    >
      <div class="dlContent">
        ...
        <span class="dlTitle">是否确定删除此小区地址</span>
      </div>
      ...
      <div class="addressDialog">
        ...
        >
          ...
          <el-form-item label="街道：" prop="street">
            <el-select
              v-model="addressForm.street" placeholder="请选择街道名称" size="medium"
              filterable clearable value-key="id"
            >
              <el-option
                v-for="item in addStreetOptions"
                :key="item.id"
                :label="item.street"
                :value="item"
              >
          ...
          <el-form-item label="小区：" prop="plot">
            <el-input v-model="addressForm.plot" placeholder="请输入小区，最长20字符" maxlength="20"></el-input>
          </el-form-item>
          <el-form-item label="总户数：" prop="totalNumber">
            <el-input v-model="addressForm.totalNumber" placeholder="请输入总户数"></el-input>
          </el-form-item>
          <el-form-item label="开卡数：" prop="cardNumber">
            <el-input v-model="addressForm.cardNumber" placeholder="请输入小区开卡数"></el-input>
          </el-form-item>
          <el-form-item label="详细地址：" prop="detailAddr">
            <el-input v-model="addressForm.detailAddr" placeholder="请输入详细地址"></el-input>
          </el-form-item>
        </el-form>
      </div>
      <span slot="footer" class="dialog-footer">
...
export default {
  data () {
    return {
      ...
      addressRules: {
        ...
        street: [
          { required: true, message: '请选择或输入街道', trigger: 'blur' }
        ],
        plot: [
  ...
  computed: {
    addArea () {
      return this.addressForm.area
    }
  },
  watch: {
    addArea: function (val) {
      if (val && val.length !== 0) {
        const params = {
          areaId: val[val.length - 1]
        }
        Server.getStreet(params).then((res) => {
          if (res.code === 0) {
          // this.$message.success(res.msg)
            this.addStreetOptions = res.result
          }
        })
      }
    }
  ...
  methods: {
    ...
    queryList () {
      ...
      if (this.plot) {
        obj.queryParams['housingEstate'] = this.plot
      }
    ...
    getAreaDetail (params) {
      Server.getAreaDetail(params).then((res) => {
        if (res.code === 0) {
          ...
          this.addressForm.street.id = this.addressDetail.streetId
          this.addressForm.street.street = this.addressDetail.street
          this.addressForm.plot = this.addressDetail.housingEstate
          this.addressForm.detailAddr = this.addressDetail.address
          this.addressForm.totalNumber = this.addressDetail.peoples
          this.addressForm.cardNumber = this.addressDetail.carNumber
        }
      })
    },
    edit (val) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          const params = {
            ...
            streetId: this.addressForm.street.id,
            street: this.addressForm.street.street,
            pid: this.addressForm.street.id,
            housingEstate: this.addressForm.plot,
            address: this.addressForm.detailAddr,
            peoples: this.addressForm.totalNumber,
            carNumber: this.addressForm.cardNumber,
            type: '2'
          }
    ...
    handleSizeChange (val) {
      ...
      if (this.plot) {
        params.queryParams['housingEstate'] = this.plot
      }
      this.getAddressList(params)
    },
